...
exercises the reference-functionality lane. From prompt v1.5, rows 1
...
| 3 | Run on a *pick: User Story whose sidecar `related:` list contains a SAME-surface Test Plan* | Related-exemplar path | `Gen_summary` shows `exemplars≥1`; the draft's case style visibly mirrors the exemplar (granularity, Positive/Negative balance); Trace lines may cite the exemplar pattern. (v2.0: only same-surface related plans land here — a cross-surface plan routes to the reference lane, row 10) |
