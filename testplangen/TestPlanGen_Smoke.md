...
checks offline on a downloaded draft). From prompt v1.6, the same
rows also check case granularity: every case carries exactly one
**Expected Result:** asserting a SINGLE outcome (a case bundling two
independently falsifiable outcomes is a fail — it should have been
split), every Steps checkbox is one tester action, and parameterized
cases name every variant they cover (never "etc." / "all types" —
the linter checks the structural half; the single-outcome and
single-action judgments are this suite's, made by reading the
draft). From prompt v1.7, rows 3 and 10 also check the source case
sweep: whenever an exemplar or reference body reached the model, the
draft carries a `## Source Case Sweep` table between Open Questions
and the Coverage Map with one row per case the source plan
describes — Yes rows citing the tailored TC, Verify rows citing
Open Questions, No rows stating why. The linter checks the table's
structure; SWEEP COMPLETENESS (no source case missing a row) is
this suite's check, made with the source plan open beside the
draft.

| # | Action | Expected | Check |
|---|---|---|---|
| 1 | Run on doc 42 ("Conflict Prevention: Acquire Locks for New Routes", User Story, Pro, 3.8) | Draft in `Shared Documents/Test Plan Drafts/`, named `TestPlanDraft__doc42__<timestamp>.md` | Banner present with source-sidecar link; the six core sections in order (Overview, Setup / Prerequisites, Positive Tests, Negative Tests, Open Questions, Coverage Map — the last added in prompt v1.5), with `Automation Notes` / `Documentation Impacts` between Negative Tests and Open Questions iff the story carries automation/documentation plans (prompt v1.2 conditional sections — never as empty headings); EVERY test case carries a **Trace:** line; Overview says surface Pro and release 3.8 verbatim; no tool named that the story doesn't name; Open Questions non-empty; every Coverage Map row's Covered by cell cites a TC id or an Open Questions entry (v1.5 — `check_draft_coverage.py` verifies); every case has exactly ONE **Expected Result:** line asserting a single outcome and single-action steps (v1.6 granularity — the linter checks the structure, this row's reader judges the semantics); `Gen_summary` counts plausible |
...
| 3 | Run on a *pick: User Story whose sidecar `related:` list contains a SAME-surface Test Plan* | Related-exemplar path | `Gen_summary` shows `exemplars≥1`; the draft's case style visibly mirrors the exemplar (granularity, Positive/Negative balance); Trace lines may cite the exemplar pattern. v1.7: the draft carries a `## Source Case Sweep` with one row per case the exemplar plan describes — open the exemplar sidecar beside the draft and count: no source case is missing a row (SWEEP COMPLETENESS); every Yes row's tailored case tests THIS story's feature (never the exemplar's), every Verify row has its Open Questions twin, every No row's reason survives scrutiny. (v2.0: only same-surface related plans land here — a cross-surface plan routes to the reference lane, row 10. v2.2: a same-surface plan past the `ExemplarSlots` count also routes to the reference lane, row 11 — never silently dropped) |
| 4 | Run on a *pick: User Story with NO Test Plan in its `related:` list* | G6 fallback path | Run history shows `Get_exemplars_q` executed; `exemplars` matches the catalog's same-surface Test Plan count (0 is a pass when none exist — draft still has all six core sections, and with BOTH lanes empty the `## Source Case Sweep` is correctly absent, v1.7) |
