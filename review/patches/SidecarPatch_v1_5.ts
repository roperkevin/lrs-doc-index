/**
 * SidecarPatch v1.5 — surgical "Related documents" patching for
 ...
 * carried through patching byte-for-byte; only the yaml `related:`
 * line and the marker region are ever rewritten, as before. Files in
 * either legacy frame patch byte-identically to v1.4 (r5 equivalence
 * leg); frame conversion remains the version-gated backfill's job.
 * ------------------------------------------------------------------
 ...
 *
 * v1.1 (frames; extended in v1.5): the metadata block comes in three
 * frames — the PromptVersion v1.9 details form (H1 + info table, then
 * ```yaml inside <details><summary>Metadata</summary>), the
 * PromptVersion v1.4 fenced form (```yaml ... ``` at position 0), and
 * the legacy `---` frontmatter form still present until the backfill
 * rewrites a file. All parse; whichever frame a file carries is
 ...
 * details frame (H1 head + <details>-wrapped ```yaml, PromptVersion
 * v1.9+), the fenced frame (```yaml at position 0, PromptVersion
 * v1.4+) and the legacy `---` frontmatter frame; returns null when
 * none opens the file or the frame never closes. The returned
 * head/open/close let the caller rebuild the file in the same frame
 * it arrived in (`head` is empty for the two legacy frames).
 */
function splitFrame(content: string): Frame | null {
  if (content.indexOf("# ") === 0) {
    // v1.5 details frame: H1 + info table head, then the wrapped yaml
    // block. The opener must sit at a line start BEFORE the first
    // section heading — a <details> block in extracted body text can
    // never be mistaken for the frame.
    const openAt = content.indexOf("\n" + DETAILS_OPEN);
    if (openAt < 0) {
      return null;
    }
    ...
    const fmStart = openAt + 1 + DETAILS_OPEN.length;
    const closeAt = content.indexOf(DETAILS_CLOSE, fmStart);
    if (closeAt < 0) {
      return null;
    }
    return {
      ...
      body: content.slice(closeAt + DETAILS_CLOSE.length),
    };
  }
  let open = "";
  let close = "";
