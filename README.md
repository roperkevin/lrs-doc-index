...
| prompts/TestPlanFigures_Prompt.md | Test-plan FIGURES prompt (`testplangen.mjs --figures`, v2.32): a second pass over a verified draft that picks the cases worth a schematic by rule (R1–R5, X1–X6, cap = `testplangen.figuresCap`, prompt v0.2's FiguresCap input, default 6) and emits grounded closed-vocabulary figure specs (v0.3 adds an optional per-route `ticks` interval); `local/lib/figurespec.mjs` grounds and renders them in the SlideFigures palette — anthropic lane verbatim, no tenant paste yet | v0.3 |
