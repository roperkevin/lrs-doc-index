...
| prompts/TestPlanFigures_Prompt.md | Test-plan FIGURES prompt (`testplangen.mjs --figures`, v2.32): a second pass over a verified draft that picks the cases worth a schematic by rule (R1–R5, X1–X6, cap = `testplangen.figuresCap`, prompt v0.2's FiguresCap input, default 6) and emits grounded closed-vocabulary figure specs; `local/lib/figurespec.mjs` grounds and renders them in the SlideFigures palette — anthropic lane verbatim, no tenant paste yet | v0.2 |
| prompts/TestPlanGen_Prompt.md | Test-plan generation prompt (deployed copy; tenant paste pending — v1.12 sends a story's preserved-value / not-updated behaviors to the Positive lane (one sentence in the Negative Tests rule); v1.11 adds the sixth input RelatedCases (retrieved catalog cases) and the VARIATION clause: a source case varying an input of a story-stated behavior is a parameterized Yes, not a Verify; v1.10 lets a case close with a story diagram's image link copied verbatim, the FIGURES rule; v1.9 makes every case's Trace cite a story statement, exemplar/reference support additive only; v1.8 makes cases name concrete fixture data with after-state tables) | v1.10 |
