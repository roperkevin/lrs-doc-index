...
| local/lib/designsystem.mjs | The deck's design systems: Fluent 2 (MIT, default), IBM Carbon (Apache 2.0) and USWDS (public domain) token blocks derived into one shape — deck type / colour roles, a 12-column grid, header / body / footer bands — each with a light and a dark theme; `restyleFigureSvg` re-colours figures to match; the thirteen-pattern layout catalog with capacities | v1.2 |
