...
| local/lib/figureindex.mjs | Figures: the standardized media names (`fig-NN[-slide-KK][-<slug>].<ext>`, minted from the extracted text so index and `--reformat` agree) + the figure parser over sidecar bodies (image rows with placement/case attribution/size/URL, diagram rows from collapsed labels) + row shaping and the FigureKey replace-set; `local/Figure_Index_Plan.md` is the design record | v1.0 |
