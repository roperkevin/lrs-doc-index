...
| local/draft2pptx.mjs | Draft .md → designed pptx review deck (zero dependencies — title/at-a-glance/divider slides, one slide per TC case with Expected Result + Trace cards, checkbox setup rows, native Coverage Map / Issue Trace tables, Diagram Style Framework palette; machine comments dropped; v1.1 renders each case's cited `**Figure:**` SVG as a figure slide via `--media <synced media dir>`) — v1.2 exports the emitter / palette / parser / package builder (notes pages) for deck2pptx, CLI guarded, output unchanged | v1.2 |
...
| local/deck2pptx.mjs | **The review deck laid out by the model** (`--generate`: one model call with prompts/TestPlanDeck_Prompt.md; `--spec`: re-render a hand-edited spec) — grounded slide by slide, laid out on the design system, rendered as native editable PowerPoint objects: text, cards, chips, checkboxes, tables, chevron flows, story / generated / inline figures as shape groups, speaker notes; zero dependencies | v1.0 |
| local/lib/designsystem.mjs | The deck's design system: Fluent 2 tokens (MIT, `@fluentui/tokens` — type ramp, spacing, radii, strokes, colour roles themed with the Diagram Style Framework palette) on the 16:9 canvas, a 12-column grid, header / body / footer bands, the thirteen-pattern layout catalog with capacities | v1.0 |
| local/lib/deckspec.mjs | Deck spec: fail-closed parse, the draft corpus (counts, cited figures), per-slide grounding with `from` references resolved (drop, never repair), the grid layout engine (pagination, nothing past the footer) | v1.0 |
