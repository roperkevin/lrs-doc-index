...
| prompts/TestPlanDeck_Prompt.md | Test-plan DECK prompt (`testplangen.mjs --deck` / `deck2pptx.mjs --generate`, v2.36): the review deck's LAYOUT decisions as a closed-vocabulary spec over thirteen design-system patterns — body content verbatim from the draft or via `from` references; `local/lib/deckspec.mjs` grounds + lays out, `local/deck2pptx.mjs` renders native editable objects — anthropic lane verbatim, no tenant paste | v0.1 |
| prompts/TestPlanFigures_Prompt.md | Test-plan FIGURES prompt (`testplangen.mjs --figures`, v2.32): a second pass over a verified draft that picks the cases worth a figure by rule (R1–R9, X1–X6, cap = `testplangen.figuresCap`, prompt v0.2's FiguresCap input, default 6) and emits grounded closed-vocabulary figure specs in eight kinds — route-measure, topology, sequence and (v0.4, TestPlanGen v2.43) timeline, state, matrix, UI wireframe, workflow — chosen by what the case asserts, with a variety clause in the budget (v0.3 adds an optional per-route `ticks` interval); `local/lib/figurespec.mjs` grounds and renders them in the SlideFigures palette — anthropic lane verbatim, no tenant paste yet | v0.4 |
| prompts/TestPlanGen_Prompt.md | Test-plan generation prompt (deployed copy; tenant paste pending — v1.13 lets a draft borrow the NAMES of a method class the story states without naming ("all input methods") from the exemplar / reference / related-cases lanes, declared once on a Setup `**Methods:**` line, the METHOD NAMES rule; v1.12 sends a story's preserved-value / not-updated behaviors to the Positive lane (one sentence in the Negative Tests rule); v1.11 adds the sixth input RelatedCases (retrieved catalog cases) and the VARIATION clause: a source case varying an input of a story-stated behavior is a parameterized Yes, not a Verify; v1.10 lets a case close with a story diagram's image link copied verbatim, the FIGURES rule; v1.9 makes every case's Trace cite a story statement, exemplar/reference support additive only; v1.8 makes cases name concrete fixture data with after-state tables) | v1.10 |
...
| pad/runner/ops.mjs | Shared script-loader/op-dispatch (used by the PAD runner and the local sweep; the `shapes` op since sweep v1.61) | v2.4 / ops v1.1 |
...
| local/svg2pptx.mjs | Figure SVGs → editable PowerPoint shapes (pull sweep figures into review decks: one native, grouped, restyleable figure per slide, titled with its case heading + source document, the case's tables as native editable tables, sidecar metadata line; zero dependencies; v1.4 exports the parser/emitter so draft2pptx embeds cited figures) — v1.5 adds `parseFigureSvg` over SVG text so deck2pptx embeds in-memory figures | v1.5 |
| local/harness/check_svg2pptx.py | svg2pptx gate (full figure-SVG vocabulary fixture; package/shape/style/label/title-band/case-table contract + sidecar lookup legs + python-pptx open leg; CI) | v1.4 |
| local/lib/*.mjs | The sweep's shared modules: util, doclinks, presentation, bodyindex, statuspage, indexpages, alerts, config (v1.31 split) + msg (CFB/.msg parser, v1.37), embedindex (embedding relatedness, v1.38), remotefs (no-OneDrive mode, v1.39), draftlint (in-process TestPlanGen draft-contract lint, testplangen v2.16), caseindex v2.1 (single TC-grammar test-case parser + replace-set planner + `caseSpans` section ranges for TestPlanGen's case-aware trimming + per-case metadata: shape, figure/table/step counts, route refs, figure links incl. the clickable FigureLink hyperlink, draft Expected Result/Trace lines, rarest-first Tools/Keywords tags from the curated vocabulary), figureindex v1.2 (standardized figure names + the Figures list parser + `placeDrawings` and the drawing fold, sweep v1.59–v1.61), figurespec v1.3 (TestPlanGen `--figures`: figure-spec reply parse, grounding verifier, SlideFigures-palette SVG renderer — route-measure / topology / sequence; v1.1 intermediate ticks, event-end measure labels, collision-free label placement; v1.2 one scale per figure and prior-state ghosts between panels; v1.3 five more kinds — timeline, state, matrix, UI wireframe, workflow — each grounded and rendered svg2pptx-convertible) | — |
| local/testplangen.mjs | **TestPlanGenCore as a local job**: story guard, related-line lanes, one model call (tenant AI Builder prompt via Dataverse Predict, or `prompts/TestPlanGen_Prompt.md` verbatim via Anthropic — per-lane via `testplangen.provider`), fail-closed marker slice, two-layer verifier (contract lint + story-grounding heuristics), `--issue`/`--title` lookup (StoryLookupFlow's queries in-process), webhook notify, timestamped draft to Shared Documents/Test Plan Drafts — `--auto`: nightly gap-drafting for freshly indexed uncovered stories (owner-switched, budget-capped, idempotent, strict-verified); the deterministic `## Issue Trace` addendum (Doc IDs + Issue Refs rows); `--gap-report`: the whole-catalog uncovered-stories digest; and `--exemplar`/`--reference`: pin human-chosen documents into the lanes ahead of the automatic related-document selection — since v1.7 `--reference` also takes an http(s) URL, feeding official web documentation (fetched, tag-stripped, defanged) into the REFERENCE FUNCTIONALITY lane with a `## Reference Documentation` addendum linking each page in the draft (Local_Setup §11; `testplangen/CHANGES.md` v2.16–v2.19, v2.22, v2.24, v2.28 — stderr progress lines + model-call heartbeat on manual runs; since v1.8 `--gap-report` also traces each story's issue ids against the sweep's Test Cases rows when that list is configured — covered-by-adjacency-only stories surface with their covering plans' case counts, v2.29; since v1.9 the case index feeds generation itself — case-traced plans fill open lane slots ahead of the G6 fallback, overflowing exemplars are trimmed whole cases at a time with the story-relevant ones kept, and every draft ends with the `## Existing Test Cases` addendum deep-linking the indexed cases that already cite the story's issues, v2.30; since v1.10 `--preview` builds every lane and stops before the model call (the zero-spend first run — inputs to workDir, `preview=1` in the summary), `--help` prints the usage, and `sweep.remoteFiles: true` mirrors the sidecar library down at run start so the job runs on a machine with no OneDrive sync; the exemplar trimmer holds ExemplarCap when the plan's head is the overflow, and draft names carry seconds so same-minute runs never overwrite, v2.31; since v1.11 `--figures` adds one more model call over the verified draft — the TestPlanFigures prompt selects the cases worth a schematic and emits grounded figure specs, `lib/figurespec.mjs` drops any that invent and renders the rest to SVG beside the draft, linked from a `## Generated Figures` addendum, `genFigures=` in Gen_summary, v2.32; since v1.12 `--stream` echoes the model's thinking summary and reply to stderr as they arrive on the anthropic lane, draft and figures calls alike, v2.33; since v1.14 the RELATED CASES lane ranks the catalog's test plans against the story and sends each top plan's best-matching cases with text plus its other case titles as prompt v1.11's sixth input, v2.34; since v1.22 `--figures` yields eight figure kinds — the TestPlanFigures prompt v0.4 picks timeline, state, matrix, UI wireframe and workflow figures beside the route schematics, the addendum names each kind and Gen_summary carries `genKinds=`, v2.43) | v1.14 |
| local/run_testplangen.cmd | Scheduled-task wrapper for `--auto --live` (inert until `testplangen.autoDraft`; self-updates from `deploy`) | — |
...
| local/harness/check_testplangen.py | Generation-job gate (mock Graph + mock Predict + mock Anthropic; guard/lanes/caps/fail-closed/verifier/lookup/notify/grounding/auto/issue-trace/gap-report/pinned-lane legs incl. draftlint↔Python agreement + web-reference + case-lane + preview/help/trimmer-head + remote-files + generated-figures + console-streaming + related-cases legs; 198 checks; CI) | — |
...
| local/draft2pptx.mjs | Draft .md → designed pptx review deck (zero dependencies — title/at-a-glance/divider slides, one slide per TC case with Expected Result + Trace cards, checkbox setup rows, native Coverage Map / Issue Trace tables, Diagram Style Framework palette; machine comments dropped; v1.1 renders each case's cited `**Figure:**` SVG as a figure slide via `--media <synced media dir>`) — v1.2 exports the emitter / palette / parser / package builder (notes pages) for deck2pptx, CLI guarded, output unchanged; v1.3 font / table / checkbox colour options for deck2pptx, output unchanged | v1.3 |
| local/harness/check_draft2pptx.py | Deck gate (python-pptx read-back: slide walk, glance counts, case-slide contract, amber VERIFY runs, native tables, provenance, CLI contract, figure slides incl. --media degrades; CI) | — |
| local/deck2pptx.mjs | **The review deck laid out by the model** (`--generate`: one model call with prompts/TestPlanDeck_Prompt.md; `--spec`: re-render a hand-edited spec) — grounded slide by slide, laid out on the design system, rendered as native editable PowerPoint objects: text, cards, chips, checkboxes, tables, chevron flows, story / generated / inline figures as shape groups, speaker notes; zero dependencies; `--design fluent/carbon/uswds` + `--theme light/dark`, figures re-coloured to match | v1.2 |
| local/lib/designsystem.mjs | The deck's design systems: Fluent 2 (MIT, default), IBM Carbon (Apache 2.0) and USWDS (public domain) token blocks derived into one shape — deck type / colour roles, a 12-column grid, header / body / footer bands — each with a light and a dark theme, every value verified against the published packages; `restyleFigureSvg` re-colours figures to match; the thirteen-pattern layout catalog with capacities | v1.3 |
| local/lib/deckspec.mjs | Deck spec: fail-closed parse, the draft corpus (counts, cited figures), per-slide grounding with `from` references resolved (drop, never repair), the grid layout engine on any design (pagination, nothing past the footer) | v1.1 |
| local/harness/check_deckspec.py | Deck module gate (fixture-free: Fluent tokens verbatim, grid arithmetic, prompt ↔ catalog agreement, fail-closed parse, corpus, fourteen drop cases, every pattern inside the canvas, pagination; CI) | — |
| local/harness/check_deck2pptx.py | Deck render gate (python-pptx read-back: the walk, the type ramp on the slide, grounds, native objects, figures as groups, notes page, amber flags, CLI, `--generate` against a mock; CI full-format) | — |
| local/harness/check_design_tokens.py | Design-token verifier (downloads @carbon/*, @fluentui/tokens and, with `--all`, @uswds/uswds from the npm registry and diffs every value in designsystem.mjs against them; manual, not CI) | — |
...
| local/Sidecar_Format_Plan.md | Sidecar format review + enhancement plan — metadata table (no yaml block), filename convention (`<issue>-<slug>.md`), extractor structure fixes, one test-case grammar, six-shape case detector, phased rollout with decisions log | all phases shipped (authored on the format branch; rollout sequence in local/CHANGES.md) |
...
| local/lib/figureindex.mjs | Figures: the standardized media names (`fig-NN[-slide-KK][-<slug>].<ext>`, minted from the extracted text so index and `--reformat` agree; untitled slides slug their first text line) + the figure parser over sidecar bodies (image rows with placement/case attribution/size/URL, icon rows for ≤ 48 px glyphs, diagram rows from collapsed labels) + row shaping and the FigureKey replace-set; `local/Figure_Index_Plan.md` is the design record | v1.1 |
