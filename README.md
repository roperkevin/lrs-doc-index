...
| Path | What it is |
|---|---|
| `pipeline/` | The jobs (`sweep`, `curate`, `testplangen`, `gantt`), the Graph/SPO client, delegated auth, the model bridge, and helper tools (`probe.mjs` list write probes, `doc_crawl.mjs` Esri help-page inventory) |
...
| `ops/` | Task Scheduler entry points (`run_sweep.cmd`, `run_curate.cmd`, `run_testplangen.cmd`, `run_heartbeat.cmd`) and the task XML |
...
All four take `--config <config.json>` and `--live | --dry-run`; the
...
Scheduling: register `ops/sweep_task.xml` (daily) and
...
- `docs/setup.md` — install, sign-in, first run, operations, each job
