...
| local/testplangen.mjs | **TestPlanGenCore as a local job**: story guard, related-line lanes, one model call (tenant AI Builder prompt via Dataverse Predict, or `prompts/TestPlanGen_Prompt.md` verbatim via Anthropic — per-lane via `testplangen.provider`), fail-closed marker slice, two-layer verifier (contract lint + story-grounding heuristics), `--issue`/`--title` lookup (StoryLookupFlow's queries in-process), webhook notify, timestamped draft to Shared Documents/Test Plan Drafts — `--auto`: nightly gap-drafting for freshly indexed uncovered stories (owner-switched, budget-capped, idempotent, strict-verified); the deterministic `## Issue Trace` addendum (Doc IDs + Issue Refs rows); and `--gap-report`: the whole-catalog uncovered-stories digest (Local_Setup §11; `testplangen/CHANGES.md` v2.16–v2.19) | v1.3 |
...
| local/harness/check_testplangen.py | Generation-job gate (mock Graph + mock Predict + mock Anthropic; guard/lanes/caps/fail-closed/verifier/lookup/notify/grounding/auto/issue-trace/gap-report legs incl. draftlint↔Python agreement; 108 checks; CI) | — |
