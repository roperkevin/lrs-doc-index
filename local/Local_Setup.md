...
5. **caseindex v1.1 columns** (2026-09-05, after the first live
   backfill): add the seven metadata columns to the live list per
   the updated `schemas/SPList_TestCases.csv` — `Shape` (Choice:
   `deck; draft`), `FigureCount`/`TableCount`/`StepCount` (Number,
   0 decimals), `RouteRefs`/`ExpectedResult`/`TraceText` (Single
   line of text). Modern list settings are fine here — none are
