...
| 1. Metadata v3 (1–2 days) | §4.1/4.2 option A; agent instructions v1.4; harness fixtures | all CI gates green; `check_local_sweep` idempotency leg proves a second `--reformat` is a no-op | `--reformat --live`, gated on `format` < 3.0 — no AI spend, whole corpus in one run |
| 1b. Filenames (1–2 days) | §4.6 slug v2, issue prefix, stem-named media folders, `_Manifest.json`, `--rename-plan` / `--rename` | dry-run table reviewed by hand once; uniqueness leg (no two rows in a folder resolve to one stem); link-rewrite leg in `check_local_sweep` (no dangling links after rename) | `--rename --live` once, same window as phase 1 |
