...
| `check_wiki.py` | fixture-free | `pipeline/wiki.mjs`: the site tree from a small sidecar library, every link resolving, the keyword canonical map, MkDocs-identical section anchors, the dialect translation (alerts, escaping, the summary and docs regions, the mkdocs extensions), the format 3.1 table, the Drafts section, the v2.0 organisation (tabs, sections, the All-documents and Browse pages, filterable tables, breadcrumbs, search front matter, magiclink/tilde/offline), the v2.1 dress by content type (the abstract / related / docs blocks, the success block, one draft box, page status, kind cards, icon titles, the palette and instant-nav knobs, the custom types in extra.css), comment/yaml hygiene, the push flow against a bare repository | Node, mkdocs-material (the strict-build legs) |
