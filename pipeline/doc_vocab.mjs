#!/usr/bin/env node
/**
 * doc_vocab.mjs v1.1 — write pipeline/data/lrs_vocabulary.json from
 ...
 * existing file (Experience Builder widgets, ribbon tools, web apps
 * and REST operations — in no toolbox, each with its `kind`) and the
 * hand-kept `aliases` map (v1.1) are carried over untouched.
 *
 * Usage:
...
async function main() {
  ...
  try {
    ...
    if (prev.aliases && typeof prev.aliases === "object") aliases = prev.aliases;
  } catch { /* first run */ }
  const out = {
    $comment: "Generated by pipeline/doc_vocab.mjs from the Esri documentation — do not edit tools/terms by hand; " +
      "re-run the script after a release. `widgets` and `aliases` are hand-kept and carried over by the script: " +
      "`widgets` = the names in no toolbox the classifier may return — Experience Builder widgets (kind widget), " +
      "Pro Location Referencing ribbon tools (kind ribbon), web apps (kind app) and the LRS REST operations (kind rest) — " +
      "as {name, kind, url}; `aliases` = shorthand → official name (SLD → Straight Line Diagram), resolved by the sweep's " +
      "normalization and by the literal text scan.",
  ...
  const line = `tools=${tools.length} terms=${terms.length} widgets=${widgets.length} aliases=${Object.keys(aliases).length} pages=${sources.length} of ${visited.size + args.vocabs.length}`;
  if (args.dry) {
