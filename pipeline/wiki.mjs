 ...
 *   pagesDeploy how that workflow publishes the built site:
 *              "actions" (default) uploads it with
 *              configure-pages / upload-pages-artifact / deploy-pages,
 *              which is github.com only — those actions go through the
 *              v4 artifact API, and on Enterprise Server the build
 *              fails outright ("@actions/artifact v2.0.0+,
 *              upload-artifact@v4+ ... are not currently supported on
 *              GHES"). "branch" instead force-pushes site/ to
 *              `pagesBranch` with plain git, which is the publishing
 *              model Enterprise Server Pages does support
 *   pagesBranch the branch "branch" mode publishes to (default
 *              "gh-pages"); point Settings -> Pages at it once
...
/* The workflow the wiki repository carries. BRANCH, RUNS_ON,
 * PAGES_BRANCH and the two step blocks are substituted at render
 * time — RUNS_ON from `wiki.runsOn`, because "ubuntu-latest" is a
 * github.com label: an Enterprise Server host (devtopia) has no
 * GitHub-hosted runners, so a job asking for it queues forever
 * instead of failing. There the label is whatever a self-hosted
 * runner advertises, usually "self-hosted". This file is REWRITTEN on
 * every run, so the label has to come from config — a hand edit in
 * the wiki repository survives exactly until the next push. */
...
/* Checkout, install, build — shared by both publish modes so the
 * plugin list cannot drift between them. */
const BUILD_STEPS =
  "    steps:\n" +
  "      - uses: actions/checkout@v4\n" +
  "SETUP_PYTHON" +
  "      - run: python -m pip install mkdocs-material mkdocs-glightbox mkdocs-panzoom-plugin markdown-captions\n" +
  "      - run: python -m mkdocs build --strict\n";

/* `wiki.pagesDeploy: "actions"` (the default) — the github.com way:
 * the build job uploads site/ as a Pages artifact and a second job
 * deploys it. */
const PAGES_WORKFLOW_ACTIONS = `name: pages
on:
  push:
    branches: [BRANCH]
  workflow_dispatch:
...
jobs:
  build:
    runs-on: RUNS_ON
BUILD_STEPS      - uses: actions/configure-pages@v5
...
/* `wiki.pagesDeploy: "branch"` — for Enterprise Server, where the
 * artifact route above cannot run at all: upload-pages-artifact tars
 * site/ and hands it to upload-artifact@v4, which refuses on GHES
 * ("@actions/artifact v2.0.0+, upload-artifact@v4+ and
 * download-artifact@v4+ are not currently supported on GHES"), and
 * deploy-pages needs the same Pages artifact API. GHES publishes from
 * a branch instead, so the build force-pushes site/ there with plain
 * git — no third-party action to sync into the instance, and the
 * built-in GITHUB_TOKEN is the only credential. The branch is a
 * rendering like everything else here: its history is one commit,
 * replaced on every run. */
const PAGES_WORKFLOW_BRANCH = `name: pages
on:
  push:
    branches: [BRANCH]
  workflow_dispatch:
...
jobs:
  build:
    runs-on: RUNS_ON
BUILD_STEPS      # Enterprise Server has no Pages artifact API, so publish the
      # built site by force-pushing it to the PAGES_BRANCH branch
      # (Settings > Pages > Source: Deploy from a branch > PAGES_BRANCH).
      - name: Publish site to PAGES_BRANCH
        shell: bash
        env:
        ...
        run: |
          ...
          remote="\${GITHUB_SERVER_URL#https://}/\${GITHUB_REPOSITORY}.git"
          git push -q --force "https://x-access-token:\${GH_TOKEN}@\${remote}" HEAD:refs/heads/PAGES_BRANCH
`;

/** The pages.yml the wiki repository carries, for the configured
 *  publish mode. PAGES_BRANCH is substituted before BRANCH, since the
 *  one contains the other. */
function pagesWorkflow(opts) {
  const tpl = opts.pagesDeploy === "branch" ? PAGES_WORKFLOW_BRANCH : PAGES_WORKFLOW_ACTIONS;
  return tpl
    .replace("BUILD_STEPS", BUILD_STEPS)
    .replaceAll("PAGES_BRANCH", opts.pagesBranch)
...
Publishing: the \`pages\` workflow builds the site on every push to \`${opts.branch}\` and publishes it to this repository's GitHub Pages${opts.pagesDeploy === "branch"
  ? ` by force-pushing the build to \`${opts.pagesBranch}\` (Settings → Pages → Source: Deploy from a branch → \`${opts.pagesBranch}\`, once). That branch is generated too — its history is one commit, replaced on every run`
...
export function renderSite(cfg, libDir, workDir, outDir, prog = noProgress) {
  ...
  const opts = {
    ...
    pagesDeploy: w.pagesDeploy || "actions",
    pagesBranch: w.pagesBranch || "gh-pages",
    recent: Number(w.recent) || 50,
    sourceSite: w.sourceSite || cfg.sweep?.siteUrl || "",
    draftsDir: w.draftsDir || "",
  };
  if (opts.pagesDeploy !== "actions" && opts.pagesDeploy !== "branch")
    throw new Error(`wiki.pagesDeploy: expected "actions" or "branch", got ${JSON.stringify(w.pagesDeploy)}`);
  /* "branch" mode force-pushes the build over wiki.pagesBranch, so the
   * two branches must differ — the same name would have the published
   * site overwrite the source tree it was rendered from. */
  if (opts.pagesDeploy === "branch" && opts.pagesBranch === opts.branch)
    throw new Error(`wiki.pagesBranch must differ from wiki.branch (both "${opts.branch}"): ` +
      "branch mode force-pushes the built site over pagesBranch, which would overwrite the site source");
