...
| **Release** | — |
| **Issues** | — |
| **Source** | [compare_calibrate_APR.docx](<https://esriis.sharepoint.com/sites/LocationReferencing/Shared%20Documents/General/Doc%20Reviews/5581_calibrate_behavior/compare_calibrate_APR.docx>) |
| **People** | author Claire Wang · PE — · dev — |
| **Edited** | 2023-11-28 20:23 by Claire Wang |
| **Extracted** | 2026-09-04 · lane xmlstrip · format 3.0 · prompt v2.0.2 |
| **Keywords** | route calibration · event behavior · calibration points · stay put · move · retire · line network · time slices |
| **Tools** | Apply Event Behaviors · Generate Routes |

## Summary

This document explains how route calibration affects events in a linear referencing system, detailing the impact of different event behaviors—Stay Put, Move, and Retire—on event measures and geographic locations. It covers scenarios for single routes and routes within a line network, describing upstream and downstream effects and providing examples with tables and images illustrating before and after calibration states.

## Related documents

<!-- related:begin -->
- [Event Behavior for Route Calibration](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/eb-for-route-calibration-rh-2023-11-2.md>) — similar text 0.92 · 4 title words · 2 filename words · same kind/surface/folder <!-- rel:449 s=8.396 -->
- [Event Behavior for Route Calibration](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/eb-for-route-calibration-apr-2023-11-2.md>) — similar text 0.80 · 4 title words · 2 filename words · same kind/surface/folder <!-- rel:448 s=7.986 -->
- [Event Behavior for Route Calibration](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/eb-for-route-calibration-rh-2023-11.md>) — similar text 0.80 · 4 title words · 1 filename word · same kind/surface/folder <!-- rel:447 s=7.486 -->
- [Event Behavior for Route Retirement](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/eb-for-route-retirement-apr-2024-01-2.md>) — similar text 0.61 · 3 title words · 1 filename word · same kind/surface <!-- rel:443 s=5.939 -->
- [Event Behavior for Route Retirement](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/eb-for-route-retirement-2024-02-2.md>) — similar text 0.51 · 3 title words · 2 filename words · same kind/surface <!-- rel:425 s=5.933 -->
<!-- related:end -->

<!-- docs:begin -->
## Esri documentation

[Event behavior](https://doc.esri.com/en/arcgis-pro/latest/help/production/location-referencing-pipelines/what-is-event-behavior.html) · [Retire routes](https://doc.esri.com/en/arcgis-pro/latest/help/production/location-referencing-pipelines/retire-routes.html)

_No page matched:_ [Apply Event Behaviors](https://www.google.com/search?q=%22Apply%20Event%20Behaviors%22+site%3Adoc.esri.com) · [Generate Routes](https://www.google.com/search?q=%22Generate%20Routes%22+site%3Adoc.esri.com)
<!-- docs:end -->
...
Calibration point changes can be added using  Location Referencing  tab  https://pro.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/add-calibration-points.htm \h calibration tools  https://pro.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/add-calibration-points.htm calibration tools, by  https://pro.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/add-calibration-points.htm  \h creating or editing calibration points using create features   https://pro.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/add-calibration-points.htm  creating or editing calibration points using create features in  ArcGIS Pro, by using  Apply downstream recalibration  in  Location Referencing  route editing tools, or by checking the  Record calibration changes for event location updates  option when running the  https://pro.arcgis.com/en/pro-app/3.2/tool-reference/location-referencing/generate-routes.htm \h Generate Routes   https://pro.arcgis.com/en/pro-app/3.2/tool-reference/location-referencing/generate-routes.htm Generate Routes tool.
...
Route editing impacts upstream and downstream sections differently. Review the following images to understand how route edits impact events based on the configured event behavior.
TheRefer to the following image showsdiagrams to understand the upstream and downstream sectionssection for the pending editroute calibration scenario:
...
If the  https://pro.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/reassign-routes.htm  \h Recalibrate route downstream option is chosen, the calibrate route event behavior is applied to the events downstream of the edited portion of the route.
If theThe network containscan contain events that span multiple routes in a  https://pro.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/essential-pipeline-referencing-vocabulary.htm  \h line network, the https://prodev.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/essential-pipeline-referencing-vocabulary.htm  \h line network. The behaviors are still applied in the same manner.
Since the LRS is  https://pro.arcgis.com/en/pro-app/3.2/help/production/location-referencing-pipelines/time-awareness-in-pipeline-referencing.htm \h time aware https://pro.arcgis.com/en/pro-app/3.1/help/production/roads-highways/time-awareness-in-roads-and-highways.htm \h time aware, edit activities—, such as calibrating or retiring a route—, time sliceslices routes and events.
...
active from 1/1/2000. The calibration is set to occur on 1/1/2005 where A calibration point is added to the route at existing measure 10 with an effective date of 1/1/2000. The added calibration point has a new measure of 15 with recalibrate downstream applied and an effective date of 1/1/2005. This calibration has the following effects:is changed to 15, resulting in recalibrated measures downstream. The graphics and tables below demonstrate the route information before and after the calibration.
...
Note:
Event1 is represented by two time slices because the routeThe recalibrated tosection starts from the nearest upstream calibration point withof the edited calibration point to the end of calibration.
...
Event2 is represented by two time slices because it crosses the location where the recalibration took place on the route. There is a time slice from 1/1/2000 to

- 1/1/2005 with the original measures from 7 to 15, and a time slice from 1/1/2005 to <Null> that stays in the same location geographically but has the new measures 9 to 20 of Route1.
...
The following image shows the route beforeand events after calibration:

The following table provides details about the events beforeafter calibration when Retire is the configured event behavior:

| Event | Route Name | From Date | To Date | From Measure | To Measure | Location Error |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Event1 |  | Route1 |  | 1/1/2000 |  | 1/1/2005 <Null> |  | 0 |  | 7 |  | No Error |  |
| Event2 |  | Route1 |  | 1/1/2000 |  | 1/1/2005 <Null> |  | 7 |  | 15 |  | No Error |  |
| Event3 |  | Route1 |  | 1/1/2000 |  | 1/1/2005 <Null> |  | 15 |  | 20 |  | No Error |  |
...
| Event ID | From Date | To Date | From RouteID Route Name | To Route ID Name | From Measure | To Measure |
| --- | --- | --- | --- | --- | --- | --- |
...
The following table provides details about the route and events before after calibration: (create this graphic by 1. Showing only 31 at the calibration:
...
##### After Move event behavior
The following image shows point location and making the routestext size and color match the original grey text; 2. Drawing a Recalibrated route section bracket from the beginning for Route3 to the end of Route3 (mimic the grey bracket above); 3. Removing the (33) from graphic and the (33) after calibration: measure from the legend)
...
| Event1 Event 1 | 1/1/2005 | <Null> | Route1 | Route3 0 | 0 Route3 | 3 0 |
...
![Figure 1 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-01-after-retire-event-behavior.png)
![Figure 2 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-02-after-retire-event-behavior.png)
![Figure 3 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-03-after-retire-event-behavior.png)
![Figure 4 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-04-after-retire-event-behavior.png)
![Figure 5 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-05-after-retire-event-behavior.png)
![Figure 6 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-06-after-retire-event-behavior.png)
![Figure 7 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-07-after-retire-event-behavior.png)
![Figure 8 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-08-after-retire-event-behavior.png)
![Figure 9 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-09-after-retire-event-behavior.png)
![Figure 10 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-10-after-retire-event-behavior.png)
![Figure 11 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-11-after-retire-event-behavior.png)
![Figure 12 — After Retire event behavior](../media/eb-for-route-calibration-apr-2023-11/fig-12-after-retire-event-behavior.png)
