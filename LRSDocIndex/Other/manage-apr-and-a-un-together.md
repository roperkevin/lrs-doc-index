...
<!-- related:begin -->
- [Manage Pipeline Referencing and a Utility Network Together](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/5048-manage-apr-and-a-un-together.md>) — similar text 0.89 · 5 title words · 1 filename word · same kind/surface <!-- rel:565 s=11.68 -->
- [Manage address and roadway characteristic data together](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/manage-address-and-roadway-characteristic-data-together.md>) — similar text 0.33 · 2 title words · 2 filename words · same kind/surface <!-- rel:96 s=5.222 -->
- [Manage Address and Roadway Characteristic Data Together](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/6267-manage-address-and-roadway-characteristic-data-together.md>) — similar text 0.34 · 2 title words · 2 filename words · same kind/surface <!-- rel:250 s=5.008 -->
- [Configure Utility Network Feature Class (Location Referencing)](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/configure-un-feature-class-lr.md>) — similar text 0.20 · 2 title words · 2 filename words · same kind/surface <!-- rel:84 s=4.991 -->
- [Manage Address and Roadway Characteristic Data Together](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/5930-manage-address-and-roadway-characteristic-data-together.md>) — similar text 0.37 · 2 title words · same kind/surface <!-- rel:327 s=4.199 -->
<!-- related:end -->
...
1. Apply an asset package (such as the gas configuration).

1. Create an LRS using one of the following tools:
...
1. If you ran the Create LRS tool, complete the following steps to configure the LRS and utility network together:

    1. Change the centerline feature class registered with the LRS from the original centerline feature class to the Pipeline Line feature class using the Modify LRS tool.

    1. Configure the Pipeline Line, Pipeline Device, and Pipeline Junction feature classes for use with an LRS using the Configure Utility Network Feature Class tool.

1. If you ran the Create LRS From Existing Dataset tool, configure the Pipeline Line, Pipeline Device, and Pipeline Junction feature classes for use with an LRS using the Configure Utility Network Feature Class tool.

1. Create LRS Networks using one of the following tools:

  - Create LRS Network
  - Create LRS Network From Existing Dataset
Learn more about creating an LRS Network

1. Create LRS events using one of the following tools:
...
1. Load data into the utility network using the Append tool and into the LRS using the Append Routes and Append Events tools.
Note:
...
1. Prepare data and publish a utility network and an LRS as a feature service.
Note:
...
1. Create and update any pipelines or centerlines intended for use in LRS editing activities:
...
1. Optionally, provide a start measure value and an end measure value for the centerlines.

1. Optionally, validate the network topology to ensure that newly created or updated pipes are valid.
...
1. Run the Apply Event Behaviors tool and any other tools necessary to update the derived network and events.

1. If other utility network feature classes have route and measure fields modeled, update them using the Update Measures From LRS tool.

1. Validate the network topology to ensure all edits are valid.

1. To add or edit LRS events, use the Location Referencing tab event editing tools in ArcGIS Pro or the Location Referencing widgets in ArcGIS Experience Builder.
