...
| **Release** | — |
| **Issues** | [ArcGISPro/ps-location-referencing#6344](https://devtopia.esri.com/ArcGISPro/ps-location-referencing/issues/6344) |
...
<!-- related:begin -->
- [Update Modify Route ID Padding topic](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/6346-update-modify-route-id-padding-topic.md>) — similar text 0.31 · 3 title words · same kind/surface <!-- rel:240 s=4.226 -->
- [Location Referencing GP Error Messages](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS%20Doc%20Index/Other/3147-lr-gp-error-messages.md>) — similar text 0.21 · same kind/surface <!-- rel:39 s=3.062 -->
- [Support Modifying and Deleting Lookup Table in an LRS Network](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/User Stories/support-modifying-and-deleting-lookup-table-in-an-lrs.md>) — similar text 0.19 · 2 title words · same surface <!-- rel:610 s=2.729 -->
- [Create Route in Line Network and Multifield Network with Additional Attributes Pro AI Assistant Test Plan](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Test Plans/create-route-in-line-network-and-multifield-network.md>) — similar text 0.06 · 1 title word · same surface <!-- rel:86 s=2.285 -->
- [Esri Roads and Highways Tutorial](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/esri-rh-tutorial.md>) — similar text 0.13 · same kind/surface <!-- rel:875 s=2.213 -->
<!-- related:end -->
...
1. In ArcGIS Pro, open the Modify Route ID Padding tool (https://prodev.arcgis.com/en/pro-app/latest/tool-reference/location-referencing/modify-route-id-padding.htm).

1. Click the LRS Network Feature Class drop-down arrow and cChoose the an LRS Nnetwork feature class with fields for padding that need to be modified.
The Route ID fields that make up the route ID automatically populate the Route ID Padding parameter the tool.

1. For each field, dDetermine the maximum number of characters any route in your LRS Nnetwork will use for each field and set the Llength value to at least that valuenumber.
...
1. Choose whether the field will be Variable Length.Specify whether the Length value is a variable or fixed value by checking or unchecking the Variable Length check box.
...
1. Check the Enable Padding check box if you want to enable padding for the field.
For this example, padding is enabled for the RouteType and RouteNumber fields is enabled.

1. Optionally, Choose specify a your pPadding cCharacter in the Padding Character text box.

- The default padding character is a space, but it can beThe padding character can be a number , space, or another character.

1. Click the Padding Location drop-down arrow and select choose an option for the field.
...
1. CCheck the Pad iIf Nullo Value Provided check box to add padding characters to the portion for that field within the RouteID and have a null value placed in the field.
when the field has a null value.

1. For this example, this check box option remains unchecked for all the fields.

- Note:
- The Allow Null Values check box is automatically checked if you check the Pad If No Value Provided check box.

1. Check the Allow Null Values check box if you want to supportto allow null values in the field.
...
1. Click Run to execute run the tool.
...
1. In ArcGIS Pro, open the Configure Lookup Table tool (https://prodev.arcgis.com/en/pro-app/latest/tool-reference/location-referencing/configure-lookup-table.htm).

1. Click the LRS Network Feature Class drop-down arrow and cChoose the an LRS Nnetwork feature class .you want to modify.

1. Click the Lookup Table drop-down arrow and cChoose the a Lookup table. from the location in your geodatabase where the table is located.
...
1. Choose a field from the Click the Field Applied To parameter drop-down arrow and choose a route ID field in the LRS Network that the lookup table is configured formenu. Only fields that are part of the multifield route ID are displayed in the drop-down options. The drop-down displays only those fields that are part of a multifield route ID for the chosen LRS Network Feature Class.
...
1. (6) Optionally, click the Lookup Display drop-down arrow and choose Select a field that contains descriptions of the  lookup keys.
Tip:
...
1. field from the Lookup Display drop-down menu.
...
1. (7) Optionally, cCheck the Allow aAny lLookup vValue check box to allow a value not present in the lookup table for the field in the concatenated RouteID. allow a value that is not in the lookup table to be added when creating a route.
...
1. 8. Click Run to execute run the tool.

Note:
Repeat the steps above if you need to configure lookup tables for multiple fields in your the LRS Nnetwork.

1. The lookup table is configured.

![Figure 1 — Configure a lookup table](../media/6344-configure-and-modify-route-id-padding-and-lookup-table/fig-01-configure-a-lookup-table.png)
![Figure 2 — Configure a lookup table](../media/6344-configure-and-modify-route-id-padding-and-lookup-table/fig-02-configure-a-lookup-table.png)
![Figure 3 — Configure a lookup table](../media/6344-configure-and-modify-route-id-padding-and-lookup-table/fig-03-configure-a-lookup-table.png)
![Figure 4 — Configure a lookup table](../media/6344-configure-and-modify-route-id-padding-and-lookup-table/fig-04-configure-a-lookup-table.png)
![Figure 5 — Configure a lookup table](../media/6344-configure-and-modify-route-id-padding-and-lookup-table/fig-05-configure-a-lookup-table.png)
