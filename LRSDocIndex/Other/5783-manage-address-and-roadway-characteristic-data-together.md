...
| **Release** | — |
| **Issues** | [ArcGISPro/ps-location-referencing#5783](https://devtopia.esri.com/ArcGISPro/ps-location-referencing/issues/5783) |
| **Source** | [5783_AddSolutionProjectInfotoADMRHConfig_V1.docx](<https://esriis.sharepoint.com/sites/LocationReferencing/Shared%20Documents/General/Doc%20Reviews/Pro35_Ent115/5783_AddSolutionProjectInfotoADMRHConfig_V1.docx>) · rev V1 |
...
<!-- related:begin -->
- [Pro 3.4 and 11.4 User Acceptance Issues and Documentation Updates](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS%20Doc%20Index/Schedules/504-pro-3-4-and-11-4-user-acceptance-issues-and-documentation.md>) — shared issue ArcGISPro/ps-location-referencing#5783 · gantt link (2 shared) · similar text 0.05 · same surface <!-- rel:194 s=1120.743 -->
- [Manage Address and Roadway Characteristic Data Together](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/5930-manage-address-and-roadway-characteristic-data-together.md>) — similar text 0.84 · 5 title words · same kind/surface <!-- rel:327 s=10.548 -->
- [Manage Address and Roadway Characteristic Data Together](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/5646-manage-address-and-roadway-characteristic-data-together.md>) — similar text 0.84 · 5 title words · same kind/surface <!-- rel:400 s=9.113 -->
- [Manage Roads and Highways with Address Data Management](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/5646-manage-rh-with-address-data-management.md>) — similar text 0.70 · 5 title words · same kind/surface <!-- rel:403 s=7.94 -->
- [Manage Address and Roadway Characteristic Data Together](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Other/6267-manage-address-and-roadway-characteristic-data-together.md>) — similar text 0.78 · 5 title words · same kind/surface <!-- rel:250 s=7.88 -->
<!-- related:end -->
...
1. Download and open the Address Data Management Pro project.

1. Create the https://prodev.arcgis.com/en/pro-app/latest/help/production/roads-highways/lrs-data-model.htmminimum schema elements for an LRS within the Address Data Management.gdb  geodatabase inside the Address fFeature dDataset, including the Calibration Point feature class, and the Redline feature classes. Create the Centerline Sequence table within the Address Data Management geodatabase.gdb.

1. Use the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/create-lrs-from-existing-dataset.htmCreate LRS From Existing Dataset tool with the Road Centerlines, Calibration Point, Redline, and Centerline Sequence layers as the inputs.

1. Create the LRS using the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/createlrs.htm \hCreate LRS or https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/create-lrs-from-existing-dataset.htm \hCreate LRS From Existing Dataset tool.

1. Run the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/configure-address-feature-classes.htm \hConfigure Address Feature Classes tool to associate the Road Centerlines and the Site Address feature classes as part of the Address Data Management solution and LRS.

1. Load data into the Road Centerlines and Site Address feature classes using the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/data-management/append.htm \hAppend tool.

### Note:

1. This step can be performed at any point between steps 3 to 6.

1. Create LRS networks using the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/create-lrs-network.htmCreate LRS Network tool. If you have a layer that you have alreadyis pre-configured as an LRS Network, use the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/create-lrs-network-from-existing-dataset.htmCreate LRS Network from Existing Dataset tool.

1. Create LRS events using the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/createlrsevent.htmCreate LRS Event tool. If you have layers that are pre-configured to be LRS events, use the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/create-lrs-event-from-existing-dataset.htmCreate LRS Event from Existing Dataset tool.

1. Load data into the LRS Network using the https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/append-routes.htm \hAppend Routes tool.and https://prodev.arcgis.com/en/pro-app/3.4/tool-reference/location-referencing/append-events.htm \hAppend Events tools.
...
1. Load data into LRS eEvents using the https://pro.arcgis.com/en/pro-app/3.3/tool-reference/location-referencing/append-events.htmAppend Events tool.

1. Change the connection file versioning type to https://prodev.arcgis.com/en/pro-app/3.4/help/data/geodatabases/overview/branch-version-scenarios.htm \hBranch, and https://prodev.arcgis.com/en/pro-app/3.4/help/production/roads-highways/migrate-an-lrs-from-a-file-geodatabase-to-a-multiuser-geodatabase.htm \hregister the data as branch versioned.

1. Note:

1. Ensure that the required fields are present to branch version the data (Global IDs and editor tracking are enabled).

1. https://prodev.arcgis.com/en/pro-app/3.4/help/production/roads-highways/share-as-web-layers.htm \hPublish an LRS in a service.

### Note:

1. When publishing an LRS in a service, This requires the  dataset to be loaded into a enterprise geodatabase connection file must use branch versioning.ed Enterprise geodatabase. Once the data is loaded, tThe Addressing feature dataset and any other required layersfeature classes and tables must be registered as branch versioned before publishing.
...
1. Create anthe LRS using the Create LRS or Create LRS From Existing Dataset tool.
...
1. Run the Configure Address Feature Classes tool to associate the newly created line event as the Site Address Point feature class.
...
1. Load data into the Site Address feature class using the Append tool.

1. Load data into the LRS using the Append Routes and Append Events tools.
...
1. Change the connection file versioning type to Branch, and register the data as branch versioned.
...
1. Provide address values in the FromLeft, ToLeft, FromRight, and ToRight fields.
...
![Figure 1 — Analysis capabilities in a combined LRS and address management dataset](../media/5783-manage-address-and-roadway-characteristic-data-together/fig-01-analysis-capabilities-in-a-combined-lrs.png)
