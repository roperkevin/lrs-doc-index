...
<!-- related:begin -->
- [Experience Builder Dynamic Segmentation widget](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/User Stories/exb-dynseg-widget.md>) — similar text 0.21 · 2 title words · 2 filename words · same kind/folder <!-- rel:362 s=3.961 -->
- [Dynamic Segmentation Table: Consider Point Events in DynSeg Table](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/User Stories/dynseg-table-consider-point-events-in-dynseg-table.md>) — similar text 0.20 · 2 title words · same kind/surface/folder <!-- rel:394 s=3.585 -->
- [Merge coincident option in DynSeg tool in Pro](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/User Stories/merge-coincident-option-in-dynseg-tool-in-pro.md>) — similar text 0.15 · 1 title word · same kind/surface/folder <!-- rel:604 s=2.933 -->
- [Rename Tool for ArcGIS Pro](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/User Stories/rename-tool-for-pro.md>) — similar text 0.21 · 1 title word · same kind/surface/folder <!-- rel:649 s=2.89 -->
- [Dynamic Segmentation Merge Option Test Plan](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Test Plans/4902-dynseg-merge-option.md>) — similar text 0.06 · 2 title words · 2 filename words <!-- rel:592 s=2.675 -->
<!-- related:end -->
...
Persona
LRS Editor: This user is responsible for making edits to the LRS. The edits they need to make come in from field crews/contractors in a variety of formats (shapefiles, engineering drawing, fgdbs). The LRS
Editor is responsible for making the route and event edits based on these documents. One workflow editors will utilize is to view the results of dynamic segmentation of events and then edit the attribute table. We supported this workflow in Event Editor and now want to support it within ArcGIS Pro for users that will do their route and event editing within a single application.
...
2.     Provide a button to ‘Configure Line Attribute Set’. This will act as a shortcut to create a line attribute set immediately.
...
2.   If any routes are already selected for the chosen network, then clicking the ‘Use Selected Routes’ button will list them under the “Routes”
...
4.   If there are routes already selected and then the Select Routes tool is used, then wipe out the previous selection and use the results of the Select Routes tool.
5.   If there are routes already selected and then the Select Routes tool is used using the SHIFT or CNTRL keys, then use the results of the Select Routes tool and add them to the previous selection
...
1.    If there are routes already selected and then the Select Routes tool is used, then wipe out the previous selection and use the results of the Select Routes tool.
2.    If there are routes already selected and then the Select Routes tool is used using the SHIFT or CNTRL keys, then use the results of the Select Routes tool and add them to the previous selection.
...
1.    Run is enabled ONLY when a network is selected, an attribute set is selected, at least one event layers is listed and at least one route is selected.
...
3.   Route ID, To Route ID (Spanning Events), From Measure, To Measure, From Date, To Date: These fields are permanent and un editable
4.   Next to the fields mentioned above are the non-lrs, non-editor tracking and non-system fields available in the event’s attribute table.
...
1.   In addition to the normal fields, allow editing the fields that have coded value domains, range domains, attribute rules, contingent values, LUT attached.
