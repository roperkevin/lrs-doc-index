...
| Field | Value |
| --- | --- |
| **Doc** | 415 · Test Plan · Experience Builder |
| **Product** | — |
| **Release** | 11.3 |
| **Issues** | [Beijing-R-D-Center/ExperienceBuilder-Web-Extensions#17309](https://devtopia.esri.com/Beijing-R-D-Center/ExperienceBuilder-Web-Extensions/issues/17309) · [ArcGISPro/ps-location-referencing#489](https://devtopia.esri.com/ArcGISPro/ps-location-referencing/issues/489) |
| **Source** | [17309-ConflictPreventionforLRSWidgets_V2.docx](<https://esriis.sharepoint.com/sites/LocationReferencing/Shared%20Documents/General/Test%20Plans/17309-ConflictPreventionforLRSWidgets_V2.docx>) · rev V2 |
...
<!-- related:begin -->
- [Test Plan for Conflict Prevention for Add Multiple Line Events Tool in ArcGIS Pro](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Test Plans/for-conflict-prevention-for-add-multiple-line-events-tool.md>) — similar text 0.73 · 2 title words · 1 filename word · same kind/folder <!-- rel:668 s=5.644 -->
- [Conflict Prevention for Event Editing in Pro – LR Event Tools](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Test Plans/conflict-prevention-for-event-editing-in-pro-lr-event-tools.md>) — similar text 0.59 · 2 title words · 1 filename word · same kind/folder <!-- rel:666 s=5.323 -->
- [Conflict Prevention for Event Editing in Pro – Add Multiple Point Events Tool Test Plan](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Test Plans/conflict-prevention-for-event-editing-in-pro-add-multiple.md>) — similar text 0.66 · 2 title words · 1 filename word · same kind/folder <!-- rel:667 s=5.139 -->
- [Conflict Prevention for Event Editing in Pro – Core Tools](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Test Plans/conflict-prevention-for-event-editing-in-pro-core-tools-v4.md>) — similar text 0.53 · 2 title words · 1 filename word · same kind/folder <!-- rel:670 s=4.762 -->
- [Conflict Prevention for Event Editing in Pro – Core Tools](<https://esriis.sharepoint.com/sites/lrsworkspace/LRS Doc Index/Test Plans/conflict-prevention-for-event-editing-in-pro-core-tools-2022-04.md>) — similar text 0.54 · 2 title words · 1 filename word · same kind/folder <!-- rel:671 s=4.634 -->
<!-- related:end -->
...
1. Test ONLY Add Point Event, Add Line Event, Split Event, and Merge Events widgets.

  1. NonLRS widgets are part of a separate user story not in the 11.3 release.

1. In each of these widgets, we will be checking/acquiring Event Locks for any events that are being edited.

1. If route lock exists for another user, attempt to transfer but if transfer fails error out the tool.

1. Test with line and non-line networks.

1. Test with point, non-spanning, and spanning events.

1. Test situations where locks are acquired, transferred, and/or released.

  1. We can only delete versions in ExB. We cannot post.

1. Ensure lock related messages in widgets are valid and understandable.

  1. Ensure existing messages are unaffected.

1. Briefly test when conflict prevention is disabled, ensure no issues arise.

1. If edit takes place in Default version, release the lock upon operation completion.

1. Follow a similar pattern for lock acquisition in the single event tools (the pattern, not the lock type) as we do for single events in Event Editor and ArcGIS Pro.

  1. Check if a lock already exists once the routeID is populated and focus changes, then try to acquire a lock when the user clicks run; if a conflicting lock already exists then have the tool/operation fail, if a lock already exists for that user/version/route/layer or no lock exists, then acquire, and let the operation succeed.

1. For the multiple event tools, we’ll need to check for the locks for the selected routeID and attribute set on the first pane of the UI when the populates the routeID and changes focus, but only acquire the lock once the user selects the event layers and clicks run on the second pane.

  1. Only lock the events that are being added in multiple. For unchecked events, do not lock or attempt to lock.

1. If a route lock already exists for a route with events being added/edited, ensure no event lock is created.

1. Auto-reconcile when necessary to acquire lock(s).

  1. To verify this, open Chrome DevTools and choose Network. Looks for startReading and startEditing. These records in the network traffic will show whether or not auto-reconcile was successful.

1. Support lock transfers in the same manner we support it in ArcGIS Pro and Event Editor
...
- **ID:** 1.No locks exist
- **Expected Result:** Event l ock can be acquired
- **Reconcile needed:** No
- **Message/s:** Message for acquiring locks at the top of pane.

### TC-U02 — 2.1. Workflow Test s: ; - If the reconcile is required, verify the prompt <!-- src: S3 · table · row 3 -->
...
### TC-U03 — Point event,; Spanning,; non- spanning event (3 . Line / Route already locked by the same user in the same version) <!-- src: S3 · table · 3 . Line / Route already locked by the same user in the same version -->

- **ID:** 3 . Line / Route already locked by the same user in the same version
- **Expected Result:** No additional lock needed
- **L1:** Locked
- **Reconcile needed:** No
- **Message/s:** No Messages

### TC-U04 — Point event,; non- spanning event (4 . Line / route already locked by another user in another version) <!-- src: S3 · table · 4 . Line / route already locked by another user in another version -->

- **ID:** 4 . Line / route already locked by another user in another version
- **Case:** Point event,; non- spanning event ,; For spanning events that user is going to add: ; - From route locked; - To route locked; - Both from / to routes are locked
- **Expected Result:** Cannot acquire locks
- **L1:** Locked
- **Reconcile needed:** No
- **Message/s:** Error message in middle of screen: Unable to add event. The Line /route L1 is already locked by UserB in VersionB1 > click ok >; Edit Failed message on tool pane in upper right corner

### TC-U05 — Point event,; Spanning,; non- spanning event (4 . 1 Line / route already locked by another user B in another version . Version not being used. User A transfers the lock and try to make an edit) <!-- src: S3 · table · 4 . 1 Line / route already locked by another user B in another version . Version not being used. User A transfers the lock and try to make an edit -->
...
- **Expected Result:** Locks can be acquired
- **L1:** Locked
- **Reconcile needed:** No
- **Message/s:** Message for acquiring locks at the upper right corner of pane.

### TC-U06 — Point event,; Spanning,; non- spanning event (5 . Concurrent route on another network is locked by the same user in the same version – verify this case with another user) <!-- src: S3 · table · 5 . Concurrent route on another network is locked by the same user in the same version – verify this case with another user -->
...
- **Expected Result:** Lock can be acquired
- **Reconcile needed:** No
- **Message/s:** Message for acquiring locks at the upper right corner of pane.

### TC-U07 — Point event,; non- spanning event (6 . Same user has a line / route lock in another version) <!-- src: S3 · table · 6 . Same user has a line / route lock in another version -->
...
- **L1:** Locked
- **Reconcile needed:** No
- **Message/s:** Error message in middle of screen: Unable to add event. The Line/route L1 is already locked by UserA in VersionA2 > click ok >; Edit Failed message on tool pane in upper right corner

### TC-U08 — Point event,; Spanning,; non- spanning event (7 . Another user has a lock (for that line / route ) on another event layer on the same route in another version) <!-- src: S3 · table · 7 . Another user has a lock (for that line / route ) on another event layer on the same route in another version -->

- **ID:** 7 . Another user has a lock (for that line / route ) on another event layer on the same route in another version
- **Expected Result:** Lock /s can be acquired
- **L1:** Locked
- **Reconcile needed:** No
- **Message/s:** Message for acquiring locks at the upper right corner of pane.

### TC-U09 — Point event,; non- spanning event (9. Event layer already locked (for that line /route ) by the same user in another version) <!-- src: S3 · table · 9. Event layer already locked (for that line /route ) by the same user in another version -->
...
- **L1:** Locked
- **Reconcile needed:** No
- **Message/s:** Error message in middle of screen:; Cannot acquire locks because UserA has the lock in version UserA.VersionA1 > click ok >; Edit Failed message on tool pane in upper right corner

### TC-U10 — Point event,; non- spanning event (1 2 . User has lock on some events, another user has lock on other events) <!-- src: S3 · table · 1 2 . User has lock on some events, another user has lock on other events -->
...
- **L1:** Locked
- **Reconcile needed:** No
- **Message/s:** Error message in middle of screen:; Cannot acquire locks because UserA has the lock in version UserA.VersionA1; +Text file enlisting all events that are locked by other user/s with the version/s info.; Edit Failed message on tool pane in upper right corner

### TC-U11 — Point event,; Spanning,; non- spanning event (1 5 . Concurrent route on another network and another event of that route are locked by the same user in the same version) <!-- src: S3 · table · 1 5 . Concurrent route on another network and another event of that route are locked by the same user in the same version -->
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  |  |  | E1 | E2 | E3 | E_N2 | E1_L1 | ES1_L1 | ES2_L1 | E1_L2 | ES1_L2 |  |  |
| 13 | Default Attribute Set (all line events)<br>All events are checked and edited in that selected Network | Some of the events are already locked by same user in another version - Event E1 of Network1, ES1_L1 of line network1 and E1_L2 of linenetwork2 are locked by User A in version A2 |  |  |  |  |  |  |  |  |  |  |  |
...
|  |  | Choosing Nonline N1 and adding only event E3 | Locked by User B in B1<br>X | x<br>(unchecked) |  |  |  |  |  |  |  | Event lock acquired for the events E3 of for the Network1 | Message for acquiring locks at the top of pane – for all events |
...
| 10e |  | All the events are locked by the same user in different version | Locked by User A in Version User B . B1 | Locked by User A in Version User B . B1 | Locked by User A in Version User B . B1 | No Event lock acquired | Error message at the top of the pane:<br>Cannot acquire locks because UserA has the lock in version UserB.B1<br>+ text file listing the locked events |
...
| 12g |  | E2 of network1 is locked by another user in same version | Choosing Network1 |  | Locked by UserB in version UserA.A1<br>(transfer) |  |  |  | Event lock acquired for the events E1, E2 & E3 of the Network1 | Message for acquiring locks at the top of pane – for all events |
...
| 13j. |  |  | Choosing Network2 |  |  |  |  |  | No Event lock acquired | Error message at the top of the pane :<br>Cannot acquire locks because User B has the lock in version User B . B1 for the event E1_N2 |
...
![Figure 1 — Split Event Wiget:](../media/17309-conflict-prevention-for-lrs-widgets/fig-01-split-event-wiget.png)
![Figure 2 — Split Event Wiget:](../media/17309-conflict-prevention-for-lrs-widgets/fig-02-split-event-wiget.png)
![Figure 3 — Split Event Wiget:](../media/17309-conflict-prevention-for-lrs-widgets/fig-03-split-event-wiget.png)
![Figure 4 — Split Event Wiget:](../media/17309-conflict-prevention-for-lrs-widgets/fig-04-split-event-wiget.png)
![Figure 5 — Split Event Wiget:](../media/17309-conflict-prevention-for-lrs-widgets/fig-05-split-event-wiget.png)
![Figure 6 — Split Event Wiget:](../media/17309-conflict-prevention-for-lrs-widgets/fig-06-split-event-wiget.png)
