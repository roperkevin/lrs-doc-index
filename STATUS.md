...
| Keyword curation | cloud flow v1.1 (still ON) — **local port authored 2026-08-15** (`local/curate.mjs`, gated 86/86; deploy = `local/Local_Setup.md` §9: set `llm.curationModelId`, smoke, schedule Saturday task, turn the cloud flow OFF — never both live) | v1.0 (`prompts/KeywordCuration_Prompt.md`) — unchanged, the local job calls the same tenant prompt | `curation/CHANGES.md` |
