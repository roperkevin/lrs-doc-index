...
Last updated: **2026-08-14 (online-doc round authored: Online Docs component v1.0 + HtmlToText v1.0 + sweep v2.9 + TestPlanGen v2.14 / prompt v1.6; r7 gate PASSED — nothing from this round is on the tenant yet)**.

## Core sweep

| Piece | Deployed | Authoritative file |
|---|---|---|
| Flow (DocIndexSweep) | **v2.7** — the 2026-08-13 live export proves the v2.6 AND v2.7 windows applied, but with 4 designer mis-picks + a stuck smoke knob (**§v2_7-fixes, apply ASAP** — image-bearing pptx docs error, keyword relateds dead, backfill stalled). **v2.8 authored** (hidden metadata comment frame, code fencing, product lines; built FROM the live export, fixes included); **v2.9 authored** (online-doc matching + sidecar surfacing over the new Online Docs list — `online_docs:` yaml + `## Online references`; PromptVersion → v2.1; requires the list to EXIST before the window, `flow/v2_9/CHANGES.md`) | live export (see `flow/v2_8/CHANGES.md`) / `flow/v2_9/definition.json` (authored) |
...
| HtmlToText | **v1.0** (r7) | **PENDING** — NEW script (no tenant version exists); standalone-safe to paste any time, consumed only by the OnlineDocFetch flow (`onlinedocs/OnlineDocs_Setup.md` §3) |
...
| check_batch_r7.py (HtmlToText v1.0 + the v2.9 sidecar-format pass-through) / check_htmltotext.py (now a standing suite, in CI) / render_sample.py (v2.9 format) / check_related.py (v2.9 byte-preservation cases) / check_draft_coverage.py (prompt v1.6 References rules, pass+fail samples) / check_batch_r6.py re-run | 2026-08-14 (see `review/harness/README.md` r7 run record) |
...
10. **Online-doc round (authored 2026-08-14, r7 gate PASSED — nothing on the tenant)**: five tenant steps, in order. (a) Build the Online Docs component per `onlinedocs/OnlineDocs_Setup.md` §§0–6 — DLP-check the premium HTTP connector first, create the list + seed + `/LRS Doc Index/Online Docs` cache folder, paste HtmlToText v1.0, build/import the OnlineDocFetch flow (re-pick the script binding, paste the list GUID), run once and spot-check one cache per host. (b) Record the new list GUID in `docs/SP_Adaptation_Notes.md`. (c) Sweep v2.9 window — sequence AFTER actions 1 and 4 (their windows are prerequisites): import `flow/DocIndexSweep_v2_9.zip` or apply designer-edits §v2_9 Y1–Y7 (paste the list GUID into Config); PromptVersion v2.1 backfills the corpus into the new sidecar layout; smoke per the §v2_9 Smoke subsection. (d) TestPlanGen v2.14 window — create the SIXTH AI Builder input parameter `OnlineDocText`, paste prompt v1.6 (replaces every pending prompt paste before it), re-import both re-cut generation zips or apply §testplangen-v2_14 Z1–Z7 (Z8 is the parameter+paste), run smoke row 12 and lint the draft with `check_draft_coverage.py`. (e) Update this file. Ordering safety: (a)–(b) are inert standalone; (d) before (c) → the lane is always `(none)` (drafts byte-identical to v1.5 behavior); (c) before (d) → sidecar data no consumer reads yet.
