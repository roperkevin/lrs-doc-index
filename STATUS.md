...
9. **TestPlanGen coverage rollout** (`testplangen/Coverage_Runbook.md`): the ordered path through the pieces of actions 1 and 8 plus the v2.0 ReferenceText contract window — live drafts stay case-thin and doc-blind until FX-3/FX-5 revive `related:` retrieval, the fifth prompt parameter exists, and the current prompt is pasted. The runbook sequences them with per-step checks and `Gen_summary` telemetry verification; work it top to bottom instead of cherry-picking the pieces.
