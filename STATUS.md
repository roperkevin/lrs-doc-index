...
| SlideFigures | **v1.3** (2026-09-03 — DF-1 slide diagrams as SVG; DF-2 multi-figure/graph lane; DF-3 routing, grid snap, legends, rotation, raster tracing; DF-4 one SVG per redraw diagram + table anchors; `check_figures.py` PASSED) | **N/A on the cloud flow** — a local-sweep-only step; a rollback simply keeps ZipTextExtract's `[figure: ...]` caption |
...
| Suite | Last green |
|---|---|
| check_local_sweep.py incl. DF-4 figure↔table placement (147/147) + check_pad_runner.py (27/27) + standing suites (check_format, check_related, check_regex, check_figures) + render_sample.py | 2026-09-03 (sweep v1.26) |
| check_figures.py (SlideFigures v1.3 / DF-4 — redraw sibling figures + anchors) | 2026-09-03 |
