...
| SlideFigures | **v1.7** (2026-09-03 — DF-1 slide diagrams as SVG; DF-2 multi-figure/graph lane; DF-3 routing, grid snap, legends, rotation, raster tracing; DF-4 one SVG per redraw diagram + table anchors; DF-5 label-collision fixes, degenerate splits, title boxes dropped; DF-6 arrowheads snapped to line tips; DF-7 spanning events as route chains; DF-8 route to the front as a dash, smaller heads on solid carriers, hash marks for labelled anchors; `check_figures.py` PASSED) | **N/A on the cloud flow** — a local-sweep-only step; a rollback simply keeps ZipTextExtract's `[figure: ...]` caption |
