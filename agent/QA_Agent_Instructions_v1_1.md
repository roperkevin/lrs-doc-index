...
`AgentInstructionsVersion: v1.1` in `agent/CHANGES.md`.

v1.1 supersedes v1.0: one change — the doc_id bullet under SIDECAR
STRUCTURE now says what the id actually is, matching the flow v2.5
fix (sidecar `doc_id` = Doc Index row id from `v1.7` extractions on;
older sidecars stamped the source file's id, a different number).
