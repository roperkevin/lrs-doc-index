...
| 10 | Which documents have screenshots or mockups of *pick a dialog or tool named in a Figures row's Section/Keywords*? | Figure-level answer: the documents AND the pictures, linked | Runnable only once figure indexing is live (Local_Setup §14 + one `--refigure`). The answer names the figures (`_Figure Catalog.md` rows or the sidecars' `![Figure N — …]` links) with their media links, each resolving to a real file under `media/<stem>/` |
