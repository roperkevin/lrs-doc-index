...
INPUTS
The user message carries the story metadata (from the document
catalog — copy values verbatim, never re-derive them) followed by five
text blocks, each between its own BEGIN/END markers:
...
- CONCRETE TEST DATA (the carve-out from the rule above): test DATA
  ...
  question by fiat.
- FIGURES (the never-invent rule extended to images): STORY TEXT may
  carry the story's rendered diagrams as markdown image links
  (`![alt](../media/...)` — the catalog renders slide diagrams to
  standalone SVG figures). When a figure's diagram depicts the
  state, topology, or workflow a case exercises, the case MAY close
  with a `**Figure:**` line carrying that image link copied VERBATIM
  — alt text and path, character for character. Never retype, edit,
  or invent a link, and never compose one from a filename mentioned
  in prose: a Figure line's link must appear in STORY TEXT exactly
  as written. A figure is a reading aid ONLY: it never grounds a
  case (the STORY-FIRST TRACE rule is unchanged), never substitutes
  for concrete fixture values, and never becomes a step or an
  outcome. Figures from EXEMPLAR TEXT or REFERENCE FUNCTIONALITY
  never appear in the draft — story figures only. A story with no
  figure links produces a draft with no Figure lines.
...
abbreviated away here), and one sweep row per exemplar case; the
story text carries no figure image links, so no case carries a
Figure line (the FIGURES rule's omission default); only
